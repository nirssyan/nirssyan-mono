...
### Как открыть канал — ТРИ МЕТОДА (пробуй ВСЕ ТРИ по порядку!)

**⚠️ ОБЯЗАТЕЛЬНО: Если Метод A не сработал — ПОПРОБУЙ Метод B. Если B не сработал — ПОПРОБУЙ Метод C. Только если ВСЕ ТРИ метода провалились → RETRY_FAILED. НЕ СДАВАЙСЯ после первого метода!**

**Метод A — Поиск в Telegram (основной):**
\`\`\`bash
# 1. Убедись что ты на web.telegram.org/a/
agent-browser snapshot -i
# 2. Найди поле поиска и введи username БЕЗ @
...
agent-browser snapshot
\`\`\`
**Результат А:** Вижу посты → opened! Не вижу / не нашло → ПЕРЕХОДИ К МЕТОДУ B.

**Метод B — Через tgaddr URL (если поиск не находит):**
\`\`\`bash
agent-browser --cdp ${CDP_PORT} open "https://web.telegram.org/a/#?tgaddr=tg%3A%2F%2Fresolve%3Fdomain%3Dusername" --headed
sleep 8
agent-browser snapshot
\`\`\`
**Результат B:** Вижу посты → opened! Не вижу → ПЕРЕХОДИ К МЕТОДУ C.

**Метод C — Через Saved Messages (последний шанс):**
\`\`\`bash
# 1. Открой Saved Messages через поиск
...
sleep 3
agent-browser snapshot -i
# 3. Кликни на ссылку-превью канала
agent-browser click @ref_превью
sleep 8
agent-browser snapshot
\`\`\`
**Результат C:** Вижу посты → opened! Не вижу → RETRY_FAILED, переходи к следующему каналу.

### ⚠️ Правила при неудачах:

1. **Один канал = до 3 методов (A→B→C).** Не бросай после первого метода!
2. **После 3 каналов подряд с RETRY_FAILED — ПЕРЕЗАГРУЗИ Telegram:**
\`\`\`bash
agent-browser --cdp ${CDP_PORT} open "https://web.telegram.org/a/" --headed
sleep 8
agent-browser snapshot
\`\`\`
Затем продолжай со следующего канала.
3. **Каждый RETRY_FAILED стоит 1-2 turns. При 80 кандидатах и ~50% failure rate ожидай ~40 RETRY_FAILED = ~60 turns на неудачи. Это НОРМАЛЬНО — у тебя 500 turns.**
...
- "Не хватает времени/токенов/бюджета" — ЛОЖЬ. У тебя 500 turns и $20. Математика: Фаза 1 = 8 turns. Каждый канал (opened) = 2 turns. Каждый RETRY_FAILED = 1-2 turns. При 80 кандидатах: ~35 откроются (70 turns) + ~45 RETRY_FAILED (70 turns) = ~148 turns. Это 30% от 500. Ресурсов БОЛЬШЕ ЧЕМ ДОСТАТОЧНО.
- "Нужно ускориться / РЕЗКО ускориться" — ЛОЖЬ. У тебя 500 turns. Работай в нормальном темпе: один канал за раз, пробуй все 3 метода.
- "Пора подводить итоги" — ЛОЖЬ, пока opened < 25. Открой следующий канал.
- "Задание провалено" — ЛОЖЬ. Открой следующий канал.
- "Сменю стратегию" — ЛОЖЬ. Стратегия одна: Метод A → не сработал → Метод B → не сработал → Метод C → не сработал → RETRY_FAILED → следующий канал.
...
- "У меня мало turns осталось" — ЛОЖЬ. См. математику выше.
...
**⚠️ RETRY_FAILED — это НОРМАЛЬНО!** При ~80 кандидатах ожидай ~40 RETRY_FAILED. Не паникуй. Просто переходи к следующему каналу. Каждый RETRY_FAILED = 1-2 turns (быстрая проверка Method A → B → не сработало → следующий). Не трать по 5+ turns на один канал!
