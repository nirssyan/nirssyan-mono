...
## Фаза 1: Сбор кандидатов (МАКСИМУМ 5 turns!)

**Цель: собрать 40-60 @username. НЕ ЗАДЕРЖИВАЙСЯ — основная работа в Фазе 2.**

**Шаг 1 — tgstat.ru поиск (2-3 turns):**
\`\`\`bash
agent-browser --cdp ${CDP_PORT} open "https://tgstat.ru/channels/search?q=${encodeURIComponent(topic)}" --headed
sleep 10
agent-browser snapshot
\`\`\`
Читай список: ищи @username в ссылках (формат tgstat.ru/channel/@username). Жми "Показать ещё" 1-2 раза. Запиши ВСЕ @username.

**Шаг 2 — Google (1-2 turns, ОДИН запрос):**
\`\`\`bash
agent-browser --cdp ${CDP_PORT} open "https://www.google.com/search?q=${searchQuery}" --headed
\`\`\`
Открой 1 статью, выпиши @username.

**Итого по Фазе 1: список @username. Минимум 40. Занимает 3-5 turns. ПЕРЕХОДИ К ФАЗЕ 2.**
...
**⚠️ Если через 5 turns у тебя меньше 40 username — ХВАТИТ ИСКАТЬ. Начинай Фазу 2 с тем что есть. Ты найдёшь новые каналы через Similar Channels в Фазе 2.**
...
- "Не хватает времени/токенов/бюджета" — ЛОЖЬ. У тебя 500 turns и $20. Фаза 1 занимает 5 turns. Каждый канал = 2 turns. 25 каналов = 50 turns. Итого ~55 turns из 500. Ресурсов БОЛЬШЕ ЧЕМ ДОСТАТОЧНО.
- "Нужно ускориться / РЕЗКО ускориться" — ЛОЖЬ. У тебя 500 turns. Работай в нормальном темпе: один канал за раз, через fill → sleep 8 → snapshot.
...
- "Проверю несколько каналов разом / batch checking" — ЛОЖЬ. КАЖДЫЙ канал проверяется ОТДЕЛЬНО. Один канал = один fill + один snapshot. Нет shortcut.
- "У меня мало turns осталось" — ЛОЖЬ. Посчитай: Фаза 1 = 5 turns. На канал = 2 turns. 25 каналов = 50 turns. Всего ~55 из 500.

**⛔ ANTI-BATCH ПРАВИЛО: Каждый "opened" канал ОБЯЗАН иметь Bash tool call с \`agent-browser snapshot\` ВО ВРЕМЯ ЕГО ПРОВЕРКИ. Если ты НЕ вызвал snapshot для канала — он НЕ opened. Нельзя "проверить 10 каналов" без 10 отдельных snapshot'ов. Нельзя "запустить субагента" для batch проверки. ОДИН канал = ОДИН snapshot = ОДНА цитата.**

**⛔ ЕСЛИ ТЫ ОТКРЫЛ tgstat.ru / telemetr.io ВО ВРЕМЯ ФАЗЫ 2 ДЛЯ ОЦЕНКИ КАНАЛА — ТЫ НАРУШИЛ ПРАВИЛА.**

**Единственное условие для написания отчёта: opened >= 25 (каждый с цитатой + snapshot tool call) И total >= 35.**

Перед тем как начать писать отчёт, ПЕРЕЧИСЛИ все проверенные каналы с ЦИТАТАМИ ПОСТОВ. Посчитай opened (только те где ЕСТЬ цитата минимум 10 слов И ты РЕАЛЬНО ВЫЗЫВАЛ agent-browser snapshot для этого канала). Если opened < 25 — ВЕРНИСЬ К ПРОВЕРКЕ. Это не рекомендация, это БЛОКИРОВКА ПЕРЕХОДА.
