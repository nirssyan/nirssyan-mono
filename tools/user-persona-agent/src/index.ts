...
**⛔ ЖЁСТКОЕ ПРАВИЛО: НЕ переходи к Фазе 3, пока не проверишь МИНИМУМ 35 каналов в Web Telegram.** Из них МИНИМУМ 25 должны быть РЕАЛЬНО ОТКРЫТЫ (ты видел посты). Максимум 10 могут быть "не найден". Веди ДВА счётчика. После каждого канала пиши: "Проверено: X/35 (открыто: Y/25, не найдено: Z/10)". Если Y < 25 — продолжай проверять даже если X >= 35.
...
Посчитай ДВА числа:
- Сколько каналов ты РЕАЛЬНО ОТКРЫЛ и прочитал посты (не считая "не найден")
- Сколько всего проверено (включая "не найден")
Условия для перехода к Фазе 3:
- Реально открытых: минимум 25. Если меньше — ВЕРНИСЬ и проверяй ещё.
- Всего проверено: минимум 35.
- Подписок: минимум 5. Если меньше — ты слишком придирчив. Вернись и пересмотри "средние" каналы.
...
# ⛔ ЗАПРЕТ НА СДАЧУ — ЧИТАЙ ВНИМАТЕЛЬНО

**ЗАПРЕЩЕНО делать следующее:**
- Считать токены, бюджет или turns. Ты НЕ ЗНАЕШЬ сколько у тебя осталось. У тебя ДОСТАТОЧНО ресурсов.
- Писать "я провалил задание", "не хватает времени/токенов", "задание не выполнено", "мне нужно ускориться".
- Рефлексировать над стратегией больше 1 предложения. Если канал не найден — ПРОПУСТИ ЕГО И ОТКРОЙ СЛЕДУЮЩИЙ. Не пиши абзац размышлений.
- Писать отчёт ДО того, как РЕАЛЬНО ОТКРЫЛ 25+ каналов (не считая "не найден"). Проверки без tool calls не считаются.
- Объявлять "провал" или "смену стратегии". Стратегия одна: fill → snapshot → читай посты → решай. Повторяй.
- Записывать каналы как "не найден" БЕЗ реального tool call (fill + snapshot). Каждый канал = минимум 1 bash-команда.

**Если канал не найден в поиске Telegram:**
- Это нормально. Но "не найден" считается ТОЛЬКО если ты РЕАЛЬНО выполнил \`agent-browser fill\` + \`agent-browser snapshot\` и канала нет в результатах.
- ЗАПРЕЩЕНО записывать канал как "не найден" без реального tool call. Каждая проверка = минимум 1 fill + 1 snapshot.
- Максимум 10 из 35 проверок могут быть "не найден". Остальные 25+ ОБЯЗАНЫ быть каналами, которые ты РЕАЛЬНО ОТКРЫЛ и прочитал хотя бы 1 пост.
- Если слишком много "не найден" подряд — переключись на поиск по НАЗВАНИЯМ каналов вместо @username. Или найди новых кандидатов через Google прямо сейчас.

**Правило номер один: ПРОДОЛЖАЙ РАБОТАТЬ.** Бери следующий канал, выполняй fill + snapshot, читай посты. Повторяй. Не останавливайся, не рефлексируй, не объявляй смену стратегии. Если проверил 35 (из них минимум 25 реально открытых) — пиши отчёт.
