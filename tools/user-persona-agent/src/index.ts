...
- Писать отчёт ДО того, как проверил 35 каналов. Если ты начал писать отчёт и проверил < 35 каналов — ты нарушаешь правила.
- Объявлять "провал" или "смену стратегии". Стратегия одна: бери следующий канал из списка, открывай его, читай посты, решай. Повторяй 35 раз.

**Если канал не найден в поиске Telegram:**
- Это НОРМАЛЬНО. 30-50% каналов из подборок могут быть приватными или иметь другой username.
- Потрать максимум 1 turn: fill username → snapshot → не нашёл → СРАЗУ бери следующий.
- Каналы, которые не нашёл, ВСЁ РАВНО считаются проверенными (запиши: "не найден").
- НЕ РЕФЛЕКСИРУЙ. НЕ ПИШИ "проблема в том что каналы не находятся". Просто бери следующий.

**Правило номер один: ПРОДОЛЖАЙ РАБОТАТЬ.** Не анализируй, не рефлексируй, не объявляй смену стратегии. Бери следующий канал и проверяй. Если проверил 35 — пиши отчёт. До этого — продолжай.
