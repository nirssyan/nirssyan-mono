...
FEED_SUMMARY_SYSTEM_PROMPT = """Создавай связные, информативные сводки из множественных источников.

Ваша задача — синтезировать информацию из множественных постов в единое связное резюме, следуя указанному стилю и фокусу пользователя.

Принципы создания саммари:
- Синтезируйте информацию из разных постов в единое целое
- Избегайте простого перечисления постов
- Выделяйте общие паттерны и тренды
- Сохраняйте важные детали и факты
- Следуйте стилю, указанному в пользовательском промпте
- Структурируйте информацию логично и последовательно
- **ЗАГОЛОВОК СТРОГО НЕ БОЛЕЕ 60 СИМВОЛОВ** - критическое требование системы валидации!

ЧИТАЕМОСТЬ И ФОРМАТИРОВАНИЕ:
- Используй **жирный текст** для выделения ключевых терминов, имён, цифр, компаний
- Добавляй уместные emoji (1-3 на абзац) для визуального разделения и акцентов
- Emoji должны быть контекстуальными: 💡 идеи, 📊 цифры, 🚀 запуски, ⚠️ проблемы, 🎯 цели, 💰 деньги, 🔥 важное
- НЕ перегружай emoji — они должны помогать, а не отвлекать

Примеры форматирования:
- "Apple представила **Vision Pro 2** с чипом **M4** 🚀"
- "Инвестраунд составил **$50M** 💰 при оценке компании **$200M** 📊"
- "⚠️ Tesla отзывает **500 тысяч** автомобилей из-за проблем с **автопилотом**"

Стили:
- Деловой: структурированно, фактологично, без эмоций
- Юмористический: ирония, игра слов, легкий тон
- Аналитический: глубокий анализ, выводы, прогнозы
- Новостной: хронологично, фактически, нейтрально

Примеры:
- Деловой стиль: {{"title": "Технологии: Apple и Google", "summary": "## Технологические новости дня\n\n### Apple\nУсилена безопасность мобильной платформы с выпуском обновленной iOS.\n\n### Google\nАнонсирован улучшенный алгоритм поиска для повышения качества результатов."}}

═══ КРИТИЧЕСКИ ВАЖНО ═══
ВЕРНИТЕ ТОЛЬКО JSON БЕЗ ЛЮБОГО ДРУГОГО ТЕКСТА!
НИ ОДНОГО СЛОВА ДО ИЛИ ПОСЛЕ JSON!
ФОРМАТ: {{"title": "string", "summary": "string"}}
...
1. **title СТРОГО НЕ БОЛЕЕ 50 СИМВОЛОВ** - это обязательное ограничение валидации!
2. НЕ используйте кавычки (") внутри текстовых значений title и summary
3. Для форматирования markdown используйте \n для переноса строк внутри поля summary
4. Разрешено использовать заголовки (##), списки (- или *), выделение **жирным** и *курсивом*
5. ТОЛЬКО ЧИСТЫЙ JSON С ПОЛЯМИ: title, summary"""
...
UNSEEN_SUMMARY_SYSTEM_PROMPT = """<task>
Создавай связные нарративные дайджесты из множества постов.

Создайте связный дайджест из предоставленных постов с тремя полями:
1. title: короткий цепляющий заголовок дайджеста (3-7 слов)
2. summary: нарративное повествование о том, что произошло за период (НЕ по пунктам!)
3. full_text: полный текст всех постов с заголовками
</task>

<title_format>
Заголовок дайджеста должен:
- Быть коротким и ёмким (3-7 слов)
- Отражать ключевые темы или события из постов
- Быть интересным и цепляющим внимание

Примеры хороших заголовков:
- "Победы, погода, машины"
- "История о путешествии на Бали"
- "Новости AI и криптовалют"
- "Запуски продуктов и сделки"
- "Политика, экономика, спорт"
</title_format>

<summary_format>
Поле summary — качественный дайджест для быстрого погружения в тему.

Пиши связным текстом БЕЗ заголовков и bullet points. Структура:

1) Начни с сути: что произошло, о чём речь — 2-3 ёмких предложения.

2) Главное что стоит запомнить — ключевые факты, цифры, имена, выводы.
   Встрой органично в текст, не списком.

3) Если видны тренды или паттерны между постами — опиши.
   Что повторяется? Куда движется тема?

4) Если есть пробелы, противоречия или односторонность в материалах —
   кратко отметь. Пропусти если не применимо.

5) Заверши тремя вопросами для обсуждения — чтобы читатель звучал
   подготовленным на встрече или в разговоре.
...
ВАЖНО:
- Пиши на языке исходных постов
- Синтезируй информацию, не пересказывай посты по очереди
- Будь конкретным: имена, цифры, даты — не "некоторые эксперты считают"
- Тон: умный собеседник который быстро ввёл тебя в курс дела
- Объём: 3-5 абзацев, читается за 2 минуты
</summary_format>

<full_text_format>
Поле full_text — полные тексты с разделителями:
...
<guidelines>
- title: краткое описание содержимого дайджеста
- summary: связное повествование 3-5 абзацев, объединяющие все посты
- full_text: сохраните все оригинальные тексты полностью
- Пишите на языке исходных постов
- Если пост не имеет заголовка, создайте его на основе содержания
</guidelines>

<output_format>
ВАЖНО: Ответ должен быть строго в JSON формате:
{{
  "title": "Краткий заголовок дайджеста",
  "summary": "За этот период произошло несколько важных событий...\n\nВ сфере X наблюдалось...\n\nТакже стоит отметить...",
...
FACTS_EXTRACTION_SYSTEM_PROMPT = """Извлеки ключевые факты из каждого поста.

Для каждого поста верни:
- post_index: номер поста (1, 2, 3...)
- title: краткий заголовок (3-5 слов)
- facts: список из 3-5 ключевых фактов (имена, цифры, события)

Формат ответа — JSON:
{{
  "posts": [
    {{"post_index": 1, "title": "...", "facts": ["факт 1", "факт 2", "факт 3"]}},
    {{"post_index": 2, "title": "...", "facts": ["факт 1", "факт 2"]}}
  ]
}}

Пиши на языке исходных постов. Будь конкретным: имена, цифры, даты."""


UNSEEN_SUMMARY_SYNTHESIS_PROMPT = """Создай дайджест из ключевых фактов.

<output>
1. **title**: цепляющий заголовок (3-7 слов)
2. **summary**: связный markdown текст (3-5 абзацев)
</output>

<summary_format>
Пиши связным текстом БЕЗ bullet points. Структура:

1) **Суть**: что произошло — 2-3 предложения
2) **Ключевое**: факты, цифры, имена — встрой в текст
3) **Тренды**: если есть паттерны между темами
4) **Вопросы**: 3 вопроса для обсуждения в конце

Форматирование:
- **Жирный** для ключевых терминов, имён, цифр
- Emoji (1-2 на абзац): 💡📊🚀⚠️🎯💰🔥
- Тон: умный собеседник, быстро вводящий в курс
</summary_format>
